...
class Twig_Extension_Application extends Twig_Extension
{
    ...
    {
		if ($value) return header('Location: ' . $value);
	}

	public function get_post($string = null)
    {
		if ($string) return $_POST[$string];
	}

	public function get_get($string = null)
    {
		if ($string) return $_GET[$string];
	}
    ...
    {
		if ($name && $value) return setcookie($name, $value, time() + 3600 * 24 * 365);
	}

	public function delete_cookie($name = null)
    {
		if ($name) return setcookie($name, '');
		
	}

	public function get_cookie($name = null)
    {
		if ($name) return $_COOKIE[$name];
	}
	...
	protected function checkin(array $args = [], array $input = [])
    {
	    foreach ($args as $key => $value) {
		    if (!$value) return ['error' => ucwords($key) . ' must ' . (is_null($value) ? 'be a string!' : 'not be empty!')]; 
        }
        if ($input) {
            $result = [];
